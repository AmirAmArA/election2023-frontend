...
export function DatePicker({city}:{city:string}) {
  ...
  const fetchRows = () => {
   ...
   const handleDateChange = (e: any)=>{
     setDate(e);
     setIsLoading(true);
     fetchByDateAndCity(e.toISOString().split('T')[0], city).then((res) => {
       setData(res);
     })
     .catch(err=>{
  ...
  return (
    <>
      ...
      {
        isLoading? <div className="flex justify-center items-center"><Loader2/></div>:<Card>
        <Table>
          <TableCaption>A list of Votes</TableCaption>
          <TableHeader>
            <TableRow>
